...
const SpaceSection = ({ spaces, onLike }: SpaceSectionProps) => {
            ...
            {spaces.map((item) => (
                ...
                >
                    <div className="relative w-[10rem] h-[7.5rem]">
                        ...
                        <Likes 
                            className={`absolute top-[0.625rem] right-[0.625rem] ${item.isLiked ? 'text-[#4CB1F1]' : 'text-white'}`}
                            onClick={() => onLike(item.placeId, item.isLiked)} />
                    </div>
                    <p 
                        className="text-[0.938rem] font-medium mt-[0.5rem]"
                        onClick={() => navigate(`/space/${item.placeId}`)}
                    >{item.name}</p>
